...
@section('left')
    ...
    <article class="page-container">
        ...
        <form  action="{{url('admin/sEdit',['slide_id'=>$fen->slide_id])}}" method="post" id="myForm" class="form form-horizontal">
            @csrf
            <div class="row cl" >
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>
                    轮播图名称：</label>
                <div class="formControls col-xs-8 col-sm-9" style="width: 30%">
                    <input type="text" id="slide_name" class="input-text" value="{{$fen->slide_name}}" placeholder="请输入轮播图名称" name="slide_name">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2">排序值：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" value="0" placeholder="{{$fen->slide_sort}}" name="slide_sort" style="width: 20%" />
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2">轮播图地址：</label>
                <div class="formControls col-xs-8 col-sm-9" style="width: 50%">
                    <input type="text" class="input-text" value="{{$fen->slide_url}}" placeholder="" id="slide_url" name="slide_url">
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9" style="width: 50%">
                    ...
                    <input type="text" size="50" name="slide_pic" value="{{$fen->slide_pic}}">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2">轮播图展示：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <img src="/{{$fen->slide_pic}}" id="slide_img" alt="">
                </div>
            </div>
            <div class="row cl">
                <div class="col-xs-12 col-sm-12 col-xs-offset-4 col-sm-offset-2">
                    <input type="submit" class="btn btn-primary btn-group-lg radius" value="修改轮播图">
                </div>
            </div>
        </form>
    </article>
