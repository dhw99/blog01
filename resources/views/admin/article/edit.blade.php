...
<article class="page-container">
	...
	<form  action="{{url('admin/aEdit',['art_id'=>$ret->art_id])}}" method="post" id="myForm" class="form form-horizontal">
		@csrf
        <div class="row cl">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" id="art_title" class="input-text" value="{{$ret->art_title}}" placeholder="请输入文章标题" name="art_title">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">作者：</label>
            <div class="formControls col-xs-8 col-sm-9" style="width: 30%">
                <input type="text" id="art_author" class="input-text" value="{{$ret->art_author}}" placeholder="请输入作者名字" name="art_author">
            </div>
        </div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">排序值：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$ret->art_sort}}" placeholder="" name="art_sort" style="width: 20%" />
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">关键词：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$ret->art_key}}" placeholder="" id="keywords" name="art_key">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">文章描述：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="art_des" cols="" rows="" class="textarea">{{$ret->art_des}}</textarea>
			</div>
		</div>
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                ...
                <input type="text" size="50" name="art_pic1" value="{{$ret->art_pic1}}">
            </div>
        </div>
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                ...
                <input type="text" size="50" name="art_pic" value="{{$ret->art_pic}}">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">图片展示：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <img src="/{{$ret->art_pic}}" id="art_img" alt="">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">修改文章：</label>
            <div class="formControls col-xs-8 col-sm-9 skin-minimal">
                <!-- 加载编辑器的容器 -->
                <script id="container" name="art_content" type="text/plain">{!!  $ret->art_content!!}</script>
		...
		<div class="row cl">
			<div class="col-xs-12 col-sm-12 col-xs-offset-4 col-sm-offset-2">
                <input type="submit" class="btn btn-primary btn-group-lg radius" value="修改文章">
			</div>
		</div>
	</form>
</article>
